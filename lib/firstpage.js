...
const IGNORABLE_ERRORS = [string => string.includes('<a href="/new">'), string => string.startsWith("<") && string.endsWith(">")];

module.exports = (playlistID, options, callback) => {
  const request = HTTPS.get(playlistID.startsWith('RD') ? `https://www.youtube.com/watch?v=piIpQlpTdXQ&list=${playlistID}&hl=en&disable_polymer=true` : `https://www.youtube.com/playlist?list=${playlistID}&hl=en&disable_polymer=true`, resp => { // eslint-disable-line consistent-return, max-len
    ...
    resp.on('end', () => { // eslint-disable-line consistent-return
      ...
      // Parse videos
      response.items = UTIL.getVideoContainers(respString, playlistID)
