...
const MIX_NAME_REGEXP = /(.*?)<\/yt-formatted-string>/
exports.getGeneralInfo = (body, plistId) => {
  if (plistId.startsWith('RD'))
    return {
      id: plistId,
      url: PLAYLIST_URL + plistId,
      title: exports.between(body, '<yt-formatted-string class="title style-scope ytd-playlist-panel-renderer">', '</yt-formatted-string>'),
      visibility: 'everyone',
      description: null,
      total_items: null,
      views: 0,
...
exports.getVideoContainers = (body, plid) => plid.startsWith('RD') ? body
  .substring(body.indexOf('<ytd-playlist-panel-video-renderer thumbnailwidth="'), body.lastIndexOf('</ytd-playlist-panel-video-renderer>'))
  .split('<ytd-playlist-panel-video-renderer')
  .splice(1) :
...
exports.buildVideoObject = videoString => {
  if (videoString.startsWith('thumbnailwidth="')) {
    return {
      ...
      title: exports.removeHtml(exports.between(videoString, '<span id="video-title" class="style-scope ytd-playlist-panel-video-renderer" title="', '"')),
      thumbnail: URL.resolve(PLAYLIST_URL, exports.between(videoString, '<img id="img" class="style-scope yt-img-shadow" alt="" width="100" src="', '"').split('?')[0]),
      duration: videoString.includes('<ytd-thumbnail-overlay-time-status-renderer') ?
        videoString.match(/<span class="style-scope ytd-thumbnail-overlay-time-status-renderer" aria-label="[^"]+">(.*?(?=<\/span>))<\/span>/)[1] :
        null,
      author: {
        name: exports.between(videoString, '<span id="byline" class="style-scope ytd-playlist-panel-video-renderer">', '</span>'),
        ref: null,
      },
    };
  }
